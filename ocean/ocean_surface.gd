...
func _process(_delta: float) -> void:
	...
	if ShelterMap.texture != null:
		_near_material.set_shader_parameter("shelter_map", ShelterMap.texture)
		_near_material.set_shader_parameter("shelter_rect", Vector4(
			ShelterMap.rect.position.x, ShelterMap.rect.position.y, ShelterMap.rect.size.x, ShelterMap.rect.size.y))
	_update_hull_masks()
