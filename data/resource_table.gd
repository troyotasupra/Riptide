...
const KINDS := {
	...
	"tree": {"label": "Chop the tree", "depleted_label": "Just a stump now — it'll grow back", "needs": "You'll need a stone hatchet to fell this — make one from wood, flint and rope (B).", "requires": "hatchet", "yields": [["log", 2, 3], ["grub", 0, 2]], "respawn": 700.0, "seconds": 4.0},
	...
	"driftwood": {"label": "Pick up driftwood", "yields": [["driftwood", 1, 2], ["grub", 0, 1]], "respawn": 900.0},
}
